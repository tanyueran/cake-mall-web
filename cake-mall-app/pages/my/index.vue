<template>
	<view class="content">
		<view class="head-wrapper" @click="toDetailHandler">
...
<script>
	...
	export default {
		...
		computed: {
			nickname() {
				if (this.$store.state.userInfo.cakeUser !== undefined) {
					return this.$store.state.userInfo.cakeUser.nickname;
				} else {
					return "--";
				}
			},
			headImg() {
				if (this.$store.state.userInfo.cakeUser !== undefined) {
					return previewFile(this.$store.state.userInfo.cakeUser.headImg);
				} else {
...
<style lang="scss">
	...
	.head-wrapper {
		display: flex;
		align-items: center;
		background: url(../../static/images/bg.jpg) left top;
		padding: 80rpx 40rpx;
		text-align: center;
